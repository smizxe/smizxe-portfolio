...
const FluidParticlesBackgroundInner = ({
  children,
  particleCount = 2000,
  ...
  useEffect(() => {
    ...
    // alpha: false → opaque canvas, avoids compositing glitches with CSS animations on siblings
    ...
    // Pre-fill canvas with dark background to avoid flash on first render
    ...
    const handleResize = () => {
      resizeCanvas();
      // Re-fill after resize to avoid transparent flash
      const isDark = forceDark || document.documentElement.classList.contains("dark");
...
// memo prevents re-render when parent state (e.g. glitchBeat) changes but props are unchanged
export const FluidParticlesBackground = memo(FluidParticlesBackgroundInner);
